...
### Use `gs://` like a local folder (FUSE)

`FUSE` is for people who want to use a Cloud Storage Bucket (i.e. upload/download to `gs://`), but do so with regular `cp`, `mv`, `rm` semantics, like the bucket were an actual local disk. Simpler than attaching a PD and simpler than using a Cloud Storage Bucket, but slower than a PD.

### Filestore file share (like FUSE, but faster)

This seems to be an evolved version of FUSE.